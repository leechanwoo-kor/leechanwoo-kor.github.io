...
![image](https://github.com/leechanwoo-kor/leechanwoo-kor.github.io/assets/55765292/79b37f6e-b392-45c5-8e34-18e958f856d6)

(2) 지점의 key가 무엇인지 명시하라.
은행(**은행명**)
소속(**은행명, 지점번호**)
지점(**지점번호**, 지점위치) -> Weak Entity?

(3) 만약 어떤 은행이 DB에서 삭제되면, 어떤 일이 발생하는지 설명하라.
- 어떤 은행이 삭제된다면 소속에서 이 은행을 참조한 tuple들은 참조 무결성 위반됨
...
- 2) 사원들 가운데 자기들이 근무하는 회사와 같은 도시에 거주하는 사원들의 이름을 찾아라
  ...
  - $RESULT \leftarrow \pi_{사원명} (사원 \bowtie_{(사원.사원명=T1.사원명) AND (사원.도시명=T1.도시명)} T1)$
