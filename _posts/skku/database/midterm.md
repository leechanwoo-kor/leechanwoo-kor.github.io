...
- 해당 은행과 관련된 모든 지점 레코드를 삭제해야 합니다.
- "은행" 엔터티의 레코드를 삭제하기 전에 연결된 "지점" 엔터티의 레코드를 모두 삭제해야 합니다. 이렇게 하면 데이터의 무결성이 유지됩니다.
- "은행"을 삭제하면 해당 은행과 연결된 모든 "지점" 레코드가 함께 삭제되므로 데이터베이스에서 관련 데이터의 일관성을 유지할 수 있습니다.
