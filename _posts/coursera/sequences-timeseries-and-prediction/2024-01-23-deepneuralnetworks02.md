...
한 번 평평해지면 쉽게 섞을 수 있습니다. `shuffle`을 호출하고 `shuffle_buffer`를 전달합니다. `shuffle_buffer`를 사용하면 속도가 조금 빨라집니다.
...
이제 이 코드로 모델을 컴파일하고 맞추겠습니다. `loss`을 MSE로 설정하여 평균 제곱 오차 함수를 사용하고 최적화 도구는 확률적 경사 하강법(SGD)을 사용할 것입니다.
