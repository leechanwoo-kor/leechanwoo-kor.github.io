...
정규화를 도입하는 경우에는 비용 함수 J의 정의를 취하고 실제로 수정했습니다. 가중치가 너무 크면 영향을 감소 시키는 추가항을 추가합니다. 그래서 만약 기울기 하강을 도입하는 경우 기울기 하강을 디버그하는 단계 중 하나는 비용 함수 J를 기울기 하강의 고도 수에 대한 함수로 표시하는 것입니다.

비용 함수 J가 단조롭게 감소하는 또 다른 정규화 기법이 가끔 사용됩니다. 그리고 정규화를 도입하는 경우, J가 새로운 뜻이 있다는 것을 기억하십시오. 만약 J의 이전 정의를 플롯하면 첫번째항에서 점진적으로 감소하는 것을 못 볼 것입니다. 따라서 기울기 하강을 디버그하려면 플로팅하고 있는지 확인하세요.

두 번째 항도 포함하는 J의 새로운 정의에선 특정 값 별로 증가할 때 마다 J가 점진적으로 감소하는 것을 못보게됩니다. L2 정규화에 대한 것입니다. 실제로 딥러닝 모델을 훈련할 때 가장 많이 사용하는 정규화 기법입니다. 딥러닝에서는 기울기 하강이 올라갈 때마다 그것은 드롭아웃 정규화라고 합니다.
