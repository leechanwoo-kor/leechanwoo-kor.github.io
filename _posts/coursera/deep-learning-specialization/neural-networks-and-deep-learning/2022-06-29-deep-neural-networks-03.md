...
입력 특성 $a^{[0]}$을 입력하 첫 번째 레이어의 활성화를 계산합니다. 이를 $a^{[1]}$라고 하고, 이를 위해 $w^{[1]}$와 $b^{[1]}$가 필요하며, $z^{[1]}$를 캐시로 저장힙니다.

이제 두 번째 레이어에 공급한 다음 $w^{[2]}, b^{[2]}$를 사용하여 다음 레이어 $a^{[2]}$에서 비활성화를 계산합니다. 결국 $a^{[l]} = \hat{y}$을 출력하게 됩니다. 그리고 이 과정에서 모든 값 $z$를 캐시했습니다. 따라서 이것이 순방향 전파 단계입니다.

이제 역방향 전파 단계에서는 역방향 반복 시퀀스를 통해 빨간 화살표를 따라 뒤로 이동하고 기울기를 계산합니다. 그래서 여기서 $da^{[l]}$을 피드하고 상자는 $da^{[l-1]}$을 줄 것이고 $da^{[2]},da^{[1]}$을 얻을 때까지 계속할 것입니다.

실제로 $da^{[0]}$를 계산하기 위해 출력을 하나 더 얻을 수 있지만 이는 입력 특징에 관한 도함수이므로 최소한 이러한 감독된 신경망의 가중치를 훈련시키는 데 유용하지는 않습니다. 그러니 여기서 멈춰도 됩니다.

하지만 역전파는 또한 $dw^{[l]},db^{[l]}$을 출력합니다. 프롬프트를 $w^{[l]},b^{[l]}$로 사용했을 뿐입니다. 그러면 $dw^{[3]},db^{[3]}$등이 출력됩니다. 따라서 필요한 모든 도함수를 계산하게 되는 거죠.

그래서 이 구조를 조금 더 채우기 위해 이러한 상자는 해당 매개변수를 사용합니다. $w^{[l]},b^{[l]}$에 말이죠. 그리고 나중에 이 상자 안에도 $dz$도 계산하게 된다는 사실을 알게 될 것입니다.

신경망을 통한 훈련의 반복 중 하나는 $x$인 $a^{[0]}$에서 시작해서 초록색 화살표를 따라 앞으로 나아가는 것을 포함합니다. $\hat{y}$을 계산한 다음 이를 사용하여 다음 항목을 계산하면 이제 모든 도함수 항들이 생성되고 $w^{[l]} := w^{[l]} - \alpha dw^{[l]}$로 업데이트됩니다. 각각의 층에 대해 그리고 $b$ 속도에 대해 유사하게 말이죠.

이제 계산된 역방향 전파에는 이 모든 도함수가 있습니다. 여기까지가 신경망 기울기 하강의 한 반복입니다.

다음으로 넘어가기 전에 한 가지 세부 정보만 더 알려드리겠습니다. 개념적으로 여기서 캐시를 역함수에 대한 $z$값을 저장하는 것으로 생각하면 유용할 것입니다. 하지만 이걸 실행해보면 프로그래밍 연습에서 볼 수 있듯이, 이를 구현하면 캐시가 $w^{[l]},b^{[l]}$을 포함하는 것을 볼 수 있습니다. 각 매개변수 값을 역함수로 가져올 수 있는 편리한 방법이 될 수 있습니다.

이 연습은 실제로 w와 b뿐만 아니라 z도 캐시에 저장합니다. 그래서 $z^{[2]},w^{[2]},b^{[2]}$를 저장합니다. 그러나 구현의 관점에서는 매개 변수를 가져와 나중에 역방향 전파를 계싼할 때 사용할 위치에 복사하는 것이 편리합니다. 이것은 프로그래밍 연습을 할 때 볼 수 있는 구현 세부 사항입니다.

지금까지 심층 신경망을 구현하기 위한 기본 빌딩 블록이 무엇인지 살펴보았습니다. 각 레이어에는 순방향 전파 단계가 있고 그에 상응하는 역방향 전파 단계가 있습니다. 그리고 정보를 서로 전달할 수 있는 캐시가 있습니다.

다음에는 이러한 빌딩 블록을 실제로 구현하는 방법에 대해 이야기할 것입니다.
