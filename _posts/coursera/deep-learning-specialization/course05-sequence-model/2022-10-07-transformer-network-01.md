...
**멀티 헤드 어텐션multi-head attention** 메커니즘에 관해 배워보도록 합시다. 표기법은 약간 복잡하지만 기본적으로 이전에 배웠던 **셀프 어텐션self attention** 메커니즘의 커다란 루프 네 개를 기억해야 합니다.

<br/>

![image](https://user-images.githubusercontent.com/55765292/194738113-67395efe-c547-4ae0-a76c-9a6a52d4083e.png)

헤드라고 불리는 시퀀스에 대해 셀프 어텐션을 계산하는 각각의 때마다 살펴보겠습니다. 멀티 헤드 어텐션이라는 이름대로 이전에 배웠던 대로 하면 되는데 어떻게 되는지 살펴보도록 하죠.

$Q, K, V$ 벡터는 각각의 입력에 몇 가지 행렬, $W^Q, W^K, W^V$을 곱하여 얻었습니다. 멀티 헤드 어텐션에서는 동일한 쿼리, 키, 값 벡터 세트를 입력으로 사용합니다. 여기 적혀 있는 $Q, K, V$ 값은 멀티 셀프 어텐션을 계산합니다.

따라서 이 중 첫째인 $Q, K, V$를 가중 행렬 $W_1^Q, W_1^K, W_1^V$에 곱합니다. 이제 이 세 개의 값이 첫 단어에 대한 새로운 쿼리, 키, 값 벡터가 됩니다. 다른 단어들에도 각각 마찬가지로 합니다. 직관성을 위해 $W_1^Q, W_1^K, W_1^V$를 질문에 묻고 답하는 데 돕기 위해 학습시킨 거라고 생각하면 유용하다는 것을 알게 되셨을 텐데 무슨 일이 일어날까요?

이건 앞서 배운 셀프 어텐션의 대략적인 예시라고 보면 됩니다. 끝나고 나면 $W_1^Q, W_1^K, W_1^V$로 무슨 일이 일어나는지 문답할 수 있길 바랍니다.

이 계산으로 단어 "visite"은 질문의 가장 올바른 답을 줍니다. 여기 이 파란색 화살표로 "l'Afrique"의 쿼리에 대해 가장 높은 값을 갖는 "visite"의 키의 내적을 나타냈고 이는 우리가 답할 첫 번째 질문입니다. 따라서 이것이 "l'Afrique"를 나타내기 위한 방법이며 "Jane", "visite", "en", "septembre"도 마찬가지입니다.

이제 시퀀스의 다섯 개의 단어를 나타내는 다섯 개의 벡터를 얻었습니다. 이것이 멀티 헤드 어텐션에 쓰이는 여러 개의 멀티 중 첫 번째에 대한 계산입니다. 이제 지금까지 한 것과 정확히 똑같은 계산을 "l'Afrique"과 다른 단어에 하여 이전과 같은, 어텐션 값 $A^{<1>}$에서 $A^{<5>}$를 얻게 됩니다.

이제부터는 한 번이 아니라 여러 번 할 것입니다. 헤드가 하나가 아니라 8개가 있으며, 이는 이 전체 계산을 8번 정도 해야 한다는 것을 의미합니다. 지금까지, 아래 첨자 1을 가진 이 행렬들로 첫 번째 헤드에 대한 어텐션을 계산했습니다. 

> $Attention(Q,K,V)= softmax	\left( \dfrac{QK^T}{\sqrt{d_k}} \right)V$

어텐션 방정식은 바로 이것인데요. 이 역시 본 적이 있을 겁니다. 이제 두 번째 헤드에 대해서도 계산을 해 봅시다. 두 번째 헤드에는 새로운 행렬이 있습니다. $W_2^Q, W_2^K, W_2^V$를 써서 이 메커니즘으로 두 번째 질문에 문답할 수 있게끔 하겠습니다.

첫 번째 질문은 "무슨 일"이 일어나는지이고, 두 번째 질문은 "언제" 그 일이 일어나는지입니다. 일반적인 사례에서 $W_1$이 여기 있는 것과는 달리 여기 $W_i$가 있고 첫 번째 멀티 뒤에 쌓인 빨간색의 두 번째 헤드가 있습니다.

이제 첫 번째 헤드와 동일한 계산을 반복하되 이 새로운 행렬로 계산하겠습니다. 이 경우에는 "september"의 키와 "l'Afrique"의 쿼리의 내적이 가장 높은 내적이 될 겁니다. "l'Afrique"에 대한 두 번째 파트에서 큰 역할을 하는 "september"의 값을 나타내기 위해 이 빨간 화살표를 강조하겠습니다.

이제 세 번째로 질문하고자 하는 것은 $W_3^Q, W_3^K, W_3^V$로 대표되는 것으로, "누가" Africa와 관련이 있는지입니다. 이 경우 세 번째 계산을 할 때 "Jane"의 키 벡터와 "l'Afrique"의 쿼리 벡터의 내적이 가장 높고 여기 이 검은 화살표로 강조됩니다. "Jane"의 값이 뒤에 쌓은 이 표시에서 가장 가중치가 큰 것을 확인할 수 있습니다.

논문에서는 헤드의 숫자는 보통 소문자 $h$로 표시됩니다. 따라서 $h$는 헤드의 수와 같습니다. 여기 이 각각의 헤드를 다른 특징이라고 생각하면 됩니다. 이러한 특징을 신경망으로 전달하면 아주 풍부한 문장 표현을 계산할 수 있습니다. 헤드가 3개든, 8개든, 그 이상이든 이 계산을 하는 데에 이러한 3개 또는 8개의 연속값이 멀티 헤드 어텐션 출력을 계산하는 데에 사용됩니다.

결국 최종 값은 이러한 모든 헤드의 연속입니다. 그런 다음 마지막으로 행렬 $W$와 곱해집니다. 기억해 둬야 할 또 한 가지 자세한 것은 멀티 헤드 어텐션을 설명하면서 각각의 헤드에 해당하는 값을 계산할 때 커다란 네 개의 루프를 쓰는 것처럼 설명했는데요. 개념적으로는 그런 식으로 이해하셔도 상관습니다.

하지만 실제로는 각 멀티 값을 병렬적으로 계산합니다. 왜냐하면 어느 것도 다른 헤드의 값에 의존하지 않기 때문입니다. 그러니 이 내용을 어떻게 구현하는지에 있어서 실제로 이 헤드를 전부 연속적으로 계산하는 대신 병렬적으로 할 수도 있습니다. 그럼 헤드의 연속에 $W_0$를 곱합니다. 그러면 멀티 헤드 어텐션이 됩니다.

셀프 어텐션에 대해 배웠고 여러 번 반복해봄으로써 이제 멀티 헤드 어텐션을 이해할 수 있게 되셨습니다. 이로써 각 단어에 대해서 여러 개의 질문을 할 수 있게 되었고, 각각의 단어에 대해 보다 풍부하고 더 좋은 표현을 학습할 수 있습니다. 이제 이걸 합쳐서 트랜스포머 네트워크를 만들어 볼 겁니다.
