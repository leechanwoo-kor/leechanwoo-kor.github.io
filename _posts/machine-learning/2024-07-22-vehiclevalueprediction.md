...
[그림 1]
그림 1: 잠재적 구매자(왼쪽)와 잠재적 판매자(오른쪽)를 위한 HCS 모바일 앱의 중고차 가격 견적 서비스 메뉴 스크린샷. 차량 번호만 입력하면 개별 차량 단위로 가격을 추정할 수 있습니다.
...
[그림 3]
그림 3: 이상 징후 탐지 알고리즘의 성능 비교.

[그림 4]
그림 4: 이상 탐지 알고리즘 비교. (검은색 점: 각 방법별로 분류된 이상치)
...
[그림 5]
그림 5: 차량 모델 임베딩을 얻기 위한 신경망 아키텍처.

[그림 6]
그림 6: 차량 임베딩 벡터의 시각화. 차량 모델은 익명화되어 있습니다.
...
[표 1]
표 1: 기본 DNN 모델과 부스트 트리 모델(XGBoost 및 CatBoost)의 차량 가격 예측 정확도 비교.

[표2]
표 2: XGBoost(XGB)와 CatBoost(CAT)를 활용한 예측 모델의 차량 모델별 성능.

[그림 7]
그림 7: 연식과 연간 주행 거리에 따라 예측된 RV 비율 비교.
...
[그림 8]
그림 8: 차량 가격 예측 모델의 자동화된 운영을 보여주는 워크플로우 다이어그램.
